...
CCA_se = NULL
CCA_p = NULL
CCA_coverage = matrix(nrow=1000, ncol=2)
...
implist_se2 = NULL
imp_pvalue1=NULL
imp_coverage1 = matrix(nrow=1000, ncol=2)
...
LOCF_se = NULL
LOCF_p = NULL
LOCF_coverage = matrix(nrow=1000, ncol=2)
...
Mean_se = NULL
Mean_p = NULL
Mean_coverage = matrix(nrow=1000, ncol=2)
...
MeanClus_se = NULL
MeanClus_p = NULL
MeanClus_coverage = matrix(nrow=1000, ncol=2)
...
#probability of missing is 20% or 0.20, on average. 
#this is missing at random, dependent upon time

#time 1 is missing 10%
Del= sample(c(0,1), size = nlength, prob=c(0.1, 0.9), replace=T)
#Creating new variable with missingness where Del=0
dtStudent1$ESA_1_20MAR = ifelse(Del==0, NA, dtStudent1$ESA_T1) 


#performing same for ESA_T2, but missing 20%
Del= sample(c(0,1), size = nlength, prob=c(0.2, 0.8), replace=T)
#Creating new variable with missingness where Del=0
dtStudent1$ESA_2_20MAR = ifelse(Del==0, NA, dtStudent1$ESA_T2)  


#performing same for ESA_T3, but missing 30% here
Del= sample(c(0,1), size = nlength, prob=c(0.3, 0.7), replace=T)
#Creating new variable with missingness where Del=0
dtStudent1$ESA_3_20MAR = ifelse(Del==0, NA, dtStudent1$ESA_T3)  # doesn't show up when larger value



#creating new dataset of just new deleted variables
Data1 = dtStudent1[, c("School", "trtGrp", "idChild", "ESA_1_20MAR", "ESA_2_20MAR", "ESA_3_20MAR", "DEIS")]
...
pred["ESA_1_20MAR", ] = c(0, 1, 0, 0, 1, 1, 0, 0)
pred["ESA_2_20MAR", ] = c(0, 1, 0, 1, 0, 1, 0, 0)
pred["ESA_3_20MAR", ] = c(0, 1, 0, 1, 1, 0, 0, 0)
pred
...
for (j in 1:40){
  impMAR20_long_01$Time=as.factor(impMAR20_long_01$Time)
  
  #linear mixed model for each imputed dataset
  lmm_mi <- lme(ESA_Recid ~ Time + trtGrp + trtGrp*Time, 
                random = list(idChild = ~1), 
                corr = corCompSymm(form= ~Time),
                data=impMAR20_long_01, subset=c(impMAR20_long_01$.imp==1), na.action="na.omit",
                method = "ML",
                control=(msMaxIter=100000))  
...
pred["ESA_1_20MAR", ] = c(-2, 1, 0, 0, 1, 1, 0, 2)
pred["ESA_2_20MAR", ] = c(-2, 1, 0, 1, 0, 1, 0, 2)
pred["ESA_3_20MAR", ] = c(-2, 1, 0, 1, 1, 0, 0, 2)
pred

#method to impute these variables is the multilevel MI for continuous variables
meth[c("ESA_1_20MAR", "ESA_2_20MAR", "ESA_3_20MAR")] = "2l.lmer"
...
SingleLevelModel1= mxModel(
  "One Level Model", type="RAM", manifestVars=c("ESA_1_20MAR", "ESA_2_20MAR", "ESA_3_20MAR", 
                                                "trtGrp"), perSchool,
...
#If missing ESA1, leave blank
#if missing in ESA_2, replace with value from ESA1
Data3$ESA_2_20MAR = ifelse(is.na(Data3$ESA_2_20MAR), Data3$ESA_1_20MAR, Data3$ESA_2_20MAR)
#if missing in ESA_3, replace with value from ESA2
Data3$ESA_3_20MAR = ifelse(is.na(Data3$ESA_3_20MAR), Data3$ESA_2_20MAR, Data3$ESA_3_20MAR)
...
Data3_long$Time = recode(Data3_long$Time, " 'ESA_1_20MAR'=1")
Data3_long$Time = recode(Data3_long$Time, " 'ESA_2_20MAR'=2")
Data3_long$Time = recode(Data3_long$Time, " 'ESA_3_20MAR'=3")
...
mean(Data4$ESA_3_20MAR[Data4$trtGrp==0], na.rm=T)

#replace each missing value as the mean of that treatment group at that time-point
Data4$ESA_1_20MAR[Data4$trtGrp==0] = ifelse(is.na(Data4$ESA_1_20MAR[Data4$trtGrp==0]),
                                             mean(Data4$ESA_1_20MAR[Data4$trtGrp==0], na.rm=T), 
                                             Data4$ESA_1_20MAR[Data4$trtGrp==0])

Data4$ESA_2_20MAR[Data4$trtGrp==0] = ifelse(is.na(Data4$ESA_2_20MAR[Data4$trtGrp==0]),
                                             mean(Data4$ESA_2_20MAR[Data4$trtGrp==0], na.rm=T), 
                                             Data4$ESA_2_20MAR[Data4$trtGrp==0])

Data4$ESA_3_20MAR[Data4$trtGrp==0] = ifelse(is.na(Data4$ESA_3_20MAR[Data4$trtGrp==0]),
                                             mean(Data4$ESA_3_20MAR[Data4$trtGrp==0], na.rm=T), 
                                             Data4$ESA_3_20MAR[Data4$trtGrp==0])

#repeat for trtgrp==1
Data4$ESA_1_20MAR[Data4$trtGrp==1] = ifelse(is.na(Data4$ESA_1_20MAR[Data4$trtGrp==1]),
                                             mean(Data4$ESA_1_20MAR[Data4$trtGrp==1], na.rm=T), 
                                             Data4$ESA_1_20MAR[Data4$trtGrp==1])

Data4$ESA_2_20MAR[Data4$trtGrp==1] = ifelse(is.na(Data4$ESA_2_20MAR[Data4$trtGrp==1]),
                                             mean(Data4$ESA_2_20MAR[Data4$trtGrp==1], na.rm=T), 
                                             Data4$ESA_2_20MAR[Data4$trtGrp==1])

Data4$ESA_3_20MAR[Data4$trtGrp==1] = ifelse(is.na(Data4$ESA_3_20MAR[Data4$trtGrp==1]),
                                             mean(Data4$ESA_3_20MAR[Data4$trtGrp==1], na.rm=T), 
                                             Data4$ESA_3_20MAR[Data4$trtGrp==1])
...
Data4_long$Time = recode(Data4_long$Time, " 'ESA_1_20MAR'=1")
Data4_long$Time = recode(Data4_long$Time, " 'ESA_2_20MAR'=2")
Data4_long$Time = recode(Data4_long$Time, " 'ESA_3_20MAR'=3")
...
for(i in 1:10){
  Data4$ESA_1_20MAR[Data4$School==i] = ifelse(is.na(Data4$ESA_1_20MAR[Data4$School==i]),
                                               mean(Data4$ESA_1_20MAR[Data4$School==i], na.rm=T), 
                                               Data4$ESA_1_20MAR[Data4$School==i])
}

for(i in 1:10){
  Data4$ESA_2_20MAR[Data4$School==i] = ifelse(is.na(Data4$ESA_2_20MAR[Data4$School==i]),
                                               mean(Data4$ESA_2_20MAR[Data4$School==i], na.rm=T), 
                                               Data4$ESA_2_20MAR[Data4$School==i])
}

for(i in 1:10){
  Data4$ESA_3_20MAR[Data4$School==i] = ifelse(is.na(Data4$ESA_3_20MAR[Data4$School==i]),
                                               mean(Data4$ESA_3_20MAR[Data4$School==i], na.rm=T), 
                                               Data4$ESA_3_20MAR[Data4$School==i])
}
...
WriteXLS(Total, ExcelFileName = "CopeSmart_10_MAR1000_20.xlsx", col.names = TRUE, row.names=TRUE)
