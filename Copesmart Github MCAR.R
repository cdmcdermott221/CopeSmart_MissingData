...
dtStudent1$ESA_1_20MCAR = ifelse(Del==0, NA, dtStudent1$ESA_T1) 


#performing same for ESA_T2
Del= sample(c(0,1), size = nlength, prob=c(0.2, 0.8), replace=T)
#Creating new variable with missingness where Del=0
dtStudent1$ESA_2_20MCAR = ifelse(Del==0, NA, dtStudent1$ESA_T2)  


#performing same for ESA_T3
Del= sample(c(0,1), size = nlength, prob=c(0.2, 0.8), replace=T)
#Creating new variable with missingness where Del=0
dtStudent1$ESA_3_20MCAR = ifelse(Del==0, NA, dtStudent1$ESA_T3)  # doesn't show up when larger value



#creating new dataset of just new deleted variables
Data1 = dtStudent1[, c("School", "trtGrp", "idChild", "ESA_1_20MCAR", "ESA_2_20MCAR", "ESA_3_20MCAR", "DEIS")]
...
#The predictors are the treatment group and the other 2 ESA variables
pred["ESA_1_20MCAR", ] = c(0, 1, 0, 0, 1, 1, 0, 0)
pred["ESA_2_20MCAR", ] = c(0, 1, 0, 1, 0, 1, 0, 0)
pred["ESA_3_20MCAR", ] = c(0, 1, 0, 1, 1, 0, 0, 0)
pred
...
for (j in 1:40){
  impMCAR20_long_01$Time=as.factor(impMCAR20_long_01$Time)
  
  #linear mixed model for each imputed dataset
  lmm_mi <- lme(ESA_Recid ~ Time + trtGrp + trtGrp*Time, 
                random = list(idChild = ~1), 
                corr = corCompSymm(form= ~Time),
                data=impMCAR20_long_01, subset=c(impMCAR20_long_01$.imp==1), na.action="na.omit",
                method = "ML",
                control=(msMaxIter=100000))  
...
pred["ESA_1_20MCAR", ] = c(-2, 1, 0, 0, 1, 1, 0, 2)
pred["ESA_2_20MCAR", ] = c(-2, 1, 0, 1, 0, 1, 0, 2)
pred["ESA_3_20MCAR", ] = c(-2, 1, 0, 1, 1, 0, 0, 2)
pred

#method to impute these variables is the multilevel MI for continuous variables
meth[c("ESA_1_20MCAR", "ESA_2_20MCAR", "ESA_3_20MCAR")] = "2l.lmer"
...
SingleLevelModel1= mxModel(
  "One Level Model", type="RAM", manifestVars=c("ESA_1_20MCAR", "ESA_2_20MCAR", "ESA_3_20MCAR", 
                                                "trtGrp"), perSchool,
...
Data3=Data1
## if missing ESA1, remains blank
#if missing in ESA_2, replace with value from ESA1
Data3$ESA_2_20MCAR = ifelse(is.na(Data3$ESA_2_20MCAR), Data3$ESA_1_20MCAR, Data3$ESA_2_20MCAR)
#if missing in ESA_3, replace with value from ESA2
Data3$ESA_3_20MCAR = ifelse(is.na(Data3$ESA_3_20MCAR), Data3$ESA_2_20MCAR, Data3$ESA_3_20MCAR)
...
Data3_long$Time = recode(Data3_long$Time, " 'ESA_1_20MCAR'=1")
Data3_long$Time = recode(Data3_long$Time, " 'ESA_2_20MCAR'=2")
Data3_long$Time = recode(Data3_long$Time, " 'ESA_3_20MCAR'=3")
...
mean(Data4$ESA_3_20MCAR[Data4$trtGrp==0], na.rm=T)

#replace each missing value as the mean of that treatment group at that time-point
Data4$ESA_1_20MCAR[Data4$trtGrp==0] = ifelse(is.na(Data4$ESA_1_20MCAR[Data4$trtGrp==0]),
                                             mean(Data4$ESA_1_20MCAR[Data4$trtGrp==0], na.rm=T), 
                                             Data4$ESA_1_20MCAR[Data4$trtGrp==0])

Data4$ESA_2_20MCAR[Data4$trtGrp==0] = ifelse(is.na(Data4$ESA_2_20MCAR[Data4$trtGrp==0]),
                                             mean(Data4$ESA_2_20MCAR[Data4$trtGrp==0], na.rm=T), 
                                             Data4$ESA_2_20MCAR[Data4$trtGrp==0])

Data4$ESA_3_20MCAR[Data4$trtGrp==0] = ifelse(is.na(Data4$ESA_3_20MCAR[Data4$trtGrp==0]),
                                             mean(Data4$ESA_3_20MCAR[Data4$trtGrp==0], na.rm=T), 
                                             Data4$ESA_3_20MCAR[Data4$trtGrp==0])

#repeat for trtgrp==1
Data4$ESA_1_20MCAR[Data4$trtGrp==1] = ifelse(is.na(Data4$ESA_1_20MCAR[Data4$trtGrp==1]),
                                             mean(Data4$ESA_1_20MCAR[Data4$trtGrp==1], na.rm=T), 
                                             Data4$ESA_1_20MCAR[Data4$trtGrp==1])

Data4$ESA_2_20MCAR[Data4$trtGrp==1] = ifelse(is.na(Data4$ESA_2_20MCAR[Data4$trtGrp==1]),
                                             mean(Data4$ESA_2_20MCAR[Data4$trtGrp==1], na.rm=T), 
                                             Data4$ESA_2_20MCAR[Data4$trtGrp==1])

Data4$ESA_3_20MCAR[Data4$trtGrp==1] = ifelse(is.na(Data4$ESA_3_20MCAR[Data4$trtGrp==1]),
                                             mean(Data4$ESA_3_20MCAR[Data4$trtGrp==1], na.rm=T), 
                                             Data4$ESA_3_20MCAR[Data4$trtGrp==1])
...
Data4_long$Time = recode(Data4_long$Time, " 'ESA_1_20MCAR'=1")
Data4_long$Time = recode(Data4_long$Time, " 'ESA_2_20MCAR'=2")
Data4_long$Time = recode(Data4_long$Time, " 'ESA_3_20MCAR'=3")
...
for(i in 1:10){
  Data4$ESA_1_20MCAR[Data4$School==i] = ifelse(is.na(Data4$ESA_1_20MCAR[Data4$School==i]),
                                               mean(Data4$ESA_1_20MCAR[Data4$School==i], na.rm=T), 
                                               Data4$ESA_1_20MCAR[Data4$School==i])
}

for(i in 1:10){
  Data4$ESA_2_20MCAR[Data4$School==i] = ifelse(is.na(Data4$ESA_2_20MCAR[Data4$School==i]),
                                               mean(Data4$ESA_2_20MCAR[Data4$School==i], na.rm=T), 
                                               Data4$ESA_2_20MCAR[Data4$School==i])
}

for(i in 1:10){
  Data4$ESA_3_20MCAR[Data4$School==i] = ifelse(is.na(Data4$ESA_3_20MCAR[Data4$School==i]),
                                               mean(Data4$ESA_3_20MCAR[Data4$School==i], na.rm=T), 
                                               Data4$ESA_3_20MCAR[Data4$School==i])
}
...
WriteXLS(Total, ExcelFileName = "CopeSmart_10_MCAR1000_20.xlsx", col.names = TRUE, row.names=TRUE)
