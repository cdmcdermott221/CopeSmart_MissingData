...
for (i in 1:1000) {
  ...
  meth[c("School", "trtGrp", "idChild", "DEIS", "constant", "ESA_1_20MNAR")] = ""
  
  #filling in predictor matrix. 1 = predictor, 0 = not a predictor
  #The predictors are the treatment group, individual-level Aux, and the other 2 ESA variables
  pred["ESA_2_20MNAR", ] = c(0, 1, 0, 1, 0, 1, 0, 0)
  pred["ESA_3_20MNAR", ] = c(0, 1, 0, 1, 1, 0, 0, 0)
  pred
  
  
  
  #method to impute these variables is for continuous variables. Doesnt take clustering into account
  meth[c("ESA_2_20MNAR", "ESA_3_20MNAR")] = "norm"
  
  #running imputation model with 40 imputed datasets
  imp = mice(Data3, meth=meth, pred=pred, maxit=100, m = 40, pri=F)
  ...
  pred["ESA_2_20MNAR", ] = c(-2, 1, 0, 1, 0, 1, 0, 2)
  pred["ESA_3_20MNAR", ] = c(-2, 1, 0, 1, 1, 0, 0, 2)
  pred
...
for(l in 1:10){
  Data4$ESA_1_20MNAR[Data4$School==l] = ifelse(is.na(Data4$ESA_1_20MNAR[Data4$School==l]),
                                               mean(Data4$ESA_1_20MNAR[Data4$School==l], na.rm=T), 
                                               Data4$ESA_1_20MNAR[Data4$School==l])
}

for(l in 1:10){
  Data4$ESA_2_20MNAR[Data4$School==l] = ifelse(is.na(Data4$ESA_2_20MNAR[Data4$School==l]),
                                               mean(Data4$ESA_2_20MNAR[Data4$School==l], na.rm=T), 
                                               Data4$ESA_2_20MNAR[Data4$School==l])
}

for(l in 1:10){
  Data4$ESA_3_20MNAR[Data4$School==l] = ifelse(is.na(Data4$ESA_3_20MNAR[Data4$School==l]),
                                               mean(Data4$ESA_3_20MNAR[Data4$School==l], na.rm=T), 
                                               Data4$ESA_3_20MNAR[Data4$School==l])
}
...
CCA_bias1 = c((Trt1_effect[1]-mean(CCA_coef[,1])), (Trt1_effect[2]-mean(CCA_se[,1])), length(which(CCA_p[,1] > 0.05)))
CCA_bias1
...
Slope1Bias = rbind(Trt1_effect, CCA_bias1, LMM_bias1, OpenMx_bias1, imp_bias1, imp_bias2, 
                   LOCF_bias2, Mean_bias2, MeanClus_bias2)
...
Total$Coverage = c(
  ...
  prop.table(table(CCA_coverage[1:100,1]))[2],
  ...
  prop.table(table(imp_coverage2[1:100,1]))[2],
  prop.table(table(LOCF_coverage[1:100,1]))[2],
  prop.table(table(Mean_coverage[1:100,1]))[2], 
  prop.table(table(MeanClus_coverage[1:100,1]))[2]
)
